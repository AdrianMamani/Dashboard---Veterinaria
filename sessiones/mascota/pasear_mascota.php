...
function obtenerMascotas() {
    ...
    if (isset($_SESSION['id_usuario'])) {
        $id_usuario = $_SESSION['id_usuario']; 
        $query = "SELECT id, nombre, especie, raza, edad, nivel_paseo FROM mascotas WHERE id_usuario = ?";
...
function actualizarPaseo($id_mascota, $incremento) {
    global $conexion;
    $query = "UPDATE mascotas SET nivel_paseo = GREATEST(LEAST(nivel_paseo + ?, 100), 0) WHERE id = ?";
    $stmt = $conexion->prepare($query);
        ...
        <tr>
            ...
            <td>
                <div class="progress-bar">
                    <div class="progress-bar-inner" style="width: <?php echo $nivel_paseo; ?>%; <?php echo $color_barra; ?>">
                        ...
                        <i class="fas fa-dog dog-walking"></i>
                    </div>
                </div>
            </td>
            ...
            <td>
                <form method="post" style="display:inline;">
                    ...
                    <button type="submit" name="pasear" class="btn-pasear">Pasear</button>
                </form>
            </td>
        </tr>
        <?php endforeach; ?>
